...
class GoogleMapsLimited extends LitElement {
  ...
  set selectedMarkerId(id) {
    ...
    let oldMarker = this._mapMarkers[this._selectedMarkerId]
    let newMarker = this._mapMarkers[id];
    if(oldMarker) oldMarker.setIcon(this.icon);
    if(newMarker) newMarker.setIcon(this.selectedIcon);
