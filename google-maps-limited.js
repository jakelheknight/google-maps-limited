...
class GoogleMapsLimited extends LitElement {
  ...
  constructor() {
    ...
    window.addEventListener('google-map-ready', () => {
      this._mapRef = new google.maps.Map(
      ...
      if (this.selectLocationMode) {
        google.maps.event.addListener(this._mapRef, 'click', event => {
          if (this._mapMarkers)
            this._mapMarkers.map(marker => marker.setMap(null));
          this.markers = [
            {
              position: { lat: event.latLng.lat(), lng: event.latLng.lng() },
              InfoWindowContent:
                this.selectedInfoWindowContent || 'User Selected Point'
            }
          ];
          this.dispatchEvent(
            new CustomEvent('lag-long-chosen', {
              bubbles: true,
              composed: true,
              detail: { lat: event.latLng.lat(), long: event.latLng.lng() }
            })
          );
  ...
  _mapScriptTag() {
    ...
    googleMapsLoader.src = `https://maps.${
      this.inChina ? 'google.cn' : 'googleapis.com'
    }/maps/api/js?key=${this.apiKey}&language=${
      lang === 'zh' ? 'zh-TW' : lang
    }&callback=initMap`;
  ...
  set selectedMarkerId(id) {
    ...
    this.dispatchEvent(
      new CustomEvent('map-pin-selected', {
        bubbles: true,
        composed: true,
        detail: { id: this.selectedMarkerId }
      })
    );
